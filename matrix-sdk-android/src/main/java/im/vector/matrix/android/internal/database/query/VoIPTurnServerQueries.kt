 ...
 */
internal fun VoIPTurnServerEntity.Companion.get(realm: Realm): VoIPTurnServerEntity? {
    return realm.where<VoIPTurnServerEntity>().findFirst()
}

/**
 * Get the current HomeServerCapabilitiesEntity, create one if it does not exist
 */
internal fun VoIPTurnServerEntity.Companion.getOrCreate(realm: Realm): VoIPTurnServerEntity {
