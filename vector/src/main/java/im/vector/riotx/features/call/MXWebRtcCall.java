...
public class MXWebRtcCall extends MXCall {
    ...
    private List<CallCandidatesContent.Candidate> mPendingCandidates = new ArrayList<>();

    private CallInviteContent mCallInviteContent = null;
     ...
     */
    public MXWebRtcCall(Session session, Context context, TurnServer turnServer, String defaultIceServerUri) {
    ...
    private void sendInvite(final SessionDescription sessionDescription) {
        ...
        CallInviteContent.Offer offerContent = new CallInviteContent.Offer(sessionDescription.type.canonicalForm(), sessionDescription.description);
        CallInviteContent inviteContent = new CallInviteContent(mCallId, 0, CALL_TIMEOUT_MS, offerContent);

        Event event = new Event(
                EventType.CALL_INVITE, null, EventKt.toContent(inviteContent, CallInviteContent.class),
                null, null, mSession.getMyUserId(), null, mCallSignalingRoom.getRoomId(), null, null
        );
    ...
    private void sendAnswer(final SessionDescription sessionDescription) {
        ...
        CallAnswerContent.Answer offerContent = new CallAnswerContent.Answer(sessionDescription.type.canonicalForm(), sessionDescription.description);
        CallAnswerContent answerContent = new CallAnswerContent(mCallId, 0, offerContent);

        Event event = new Event(
                EventType.CALL_ANSWER, null, EventKt.toContent(answerContent, CallAnswerContent.class),
                null, null, mSession.getMyUserId(), null, mCallSignalingRoom.getRoomId(), null, null);
        mPendingEvents.add(event);
        sendNextEvent();
        ...
        mPeerConnection = mPeerConnectionFactory.createPeerConnection(
                ...
                new PeerConnection.Observer() {
                    ...
                    public void onIceCandidate(final IceCandidate iceCandidate) {
                        ...
                        mUIThreadHandler.post(() -> {
                            if (!isCallEnded()) {
                                CallCandidatesContent.Candidate candidate = new CallCandidatesContent.Candidate(iceCandidate.sdpMid, Integer.toString(iceCandidate.sdpMLineIndex), iceCandidate.sdp);
                                CallCandidatesContent candidateContent = new CallCandidatesContent(mCallId, 0, Collections.singletonList(candidate));
                                ...
                                if (mPendingEvents.size() > 0) {
                                    try {
                                        ...
                                        if (EventType.CALL_CANDIDATES.equals(lastEvent.getType())) {
                                            mPendingEvents.remove(lastEvent);
                                            CallCandidatesContent lastContent = EventKt.toModel(lastEvent.getClearContent(), CallCandidatesContent.class, true);

                                            List<CallCandidatesContent.Candidate> lastContentCandidates = lastContent.getCandidates();
                                            List<CallCandidatesContent.Candidate> newContentCandidates = candidateContent.getCandidates();
                                            ...
                                            candidateContent = new CallCandidatesContent(lastContent.getCallId(), lastContent.getVersion(), lastContentCandidates);
                                        }
                                    } catch (Exception e) {
                                ...
                                Event event = new Event(EventType.CALL_CANDIDATES, null, EventKt.toContent(candidateContent, CallCandidatesContent.class),
                                        null, null, mSession.getMyUserId(), null, mCallSignalingRoom.getRoomId(), null, null);
     ...
     *
     * @param callInviteContent    the invite params
     * @param aLocalVideoPosition position of the local video attendee
     */
    @SuppressLint("deprecation")
    private void initCallUI(final CallInviteContent callInviteContent, VideoLayoutConfiguration aLocalVideoPosition) {
        ...
        if (isVideo()) {
                ...
                mUIThreadHandler.post(() -> {
                    if (null == mPeerConnectionFactory) {
                        ...
                        */
                        ...
                        if (null != callInviteContent) {
                            dispatchOnStateDidChange(CALL_STATE_RINGING);
                            setRemoteDescription(callInviteContent);
                        }
                    }
                });
            ...
            try {
                ...
                mFullScreenRTCView.setEglBase(eglBase);
                ...
                mPipRTCView.setEglBase(eglBase);
     ...
     *
     * @param callInviteContent the invitation params
     */
    private void setRemoteDescription(final CallInviteContent callInviteContent) {
        Log.d(LOG_TAG, "setRemoteDescription " + callInviteContent);

        SessionDescription aDescription = null;
        // extract the description
        try {
            CallInviteContent.Offer offer = callInviteContent.getOffer();
            if (offer != null) {
                if (!TextUtils.isEmpty(offer.getType()) && !TextUtils.isEmpty(offer.getSdp())) {
                    aDescription = new SessionDescription(SessionDescription.Type.OFFER, offer.getSdp());
                }
            }
     ...
     */
    @Override
    public void prepareIncomingCall(final CallInviteContent callInviteContent, final String aCallId, final VideoLayoutConfiguration aLocalVideoPosition) {
        Log.d(LOG_TAG, "## prepareIncomingCall : call state " + getCallState());
        super.prepareIncomingCall(callInviteContent, aCallId, aLocalVideoPosition);
        mCallId = aCallId;
        ...
        if (CALL_STATE_READY.equals(getCallState())) {
            ...
            mUIThreadHandler.post(() -> initCallUI(callInviteContent, aLocalVideoPosition));
        } else if (CALL_STATE_CREATED.equals(getCallState())) {
            mCallInviteContent = callInviteContent;

            // detect call type from the sdp
            try {
                setIsVideo(callInviteContent.getOffer().getSdp().contains("m=video"));
            } catch (Exception e) {
    ...
    public void launchIncomingCall(VideoLayoutConfiguration aLocalVideoPosition) {
        ...
        if (CALL_STATE_READY.equals(getCallState())) {
            prepareIncomingCall(mCallInviteContent, mCallId, aLocalVideoPosition);
        }
    }
    ...
    private void onCallAnswer(final Event event) {
        ...
        if (!CALL_STATE_CREATED.equals(getCallState()) && (null != mPeerConnection)) {
            mUIThreadHandler.post(() -> {
                ...
                // extract the description
                if (EventType.CALL_ANSWER.equals(event.getType())) {
                    CallAnswerContent eventContent = EventKt.toModel(event.getClearContent(), CallAnswerContent.class, true);

                    if (eventContent != null && eventContent.getAnswer() != null) {
                        String type = eventContent.getAnswer().getType();
                        String sdp = eventContent.getAnswer().getSdp();
     ...
     */
    private void onNewCandidates(final List<CallCandidatesContent.Candidate> candidates) {
        ...
        if (!CALL_STATE_CREATED.equals(getCallState()) && (null != mPeerConnection)) {
            ...
            for (CallCandidatesContent.Candidate item: candidates) {
                try {
                    String candidate = item.getCandidate();
                    String sdpMid = item.getSdpMid();
                    int sdpLineIndex = Integer.parseInt(item.getSdpMLineIndex());
     ...
     */
    private void addCandidates(List<CallCandidatesContent.Candidate> candidates) {
    ...
    private void checkPendingCandidates() {
        ...
        synchronized (LOG_TAG) {
            ...
            mPendingCandidates = Collections.emptyList();
        }
    }
    ...
    public void handleCallEvent(Event event) {
        ...
        if (EventKt.isCallEvent(event)) {
            String eventType = event.getType();

            Log.d(LOG_TAG, "handleCallEvent " + eventType);

            // event from other member
            if (! mSession.getMyUserId().equals(event.getSenderId())) {
                if (EventType.CALL_ANSWER.equals(eventType) && !mIsIncoming) {
                    onCallAnswer(event);
                } else if (EventType.CALL_CANDIDATES.equals(eventType)) {
                    CallCandidatesContent eventContent = EventKt.toModel(event.getClearContent(), CallCandidatesContent.class, true);
                    addCandidates(eventContent.getCandidates());
                } else if (EventType.CALL_HANGUP.equals(eventType)) {
                    onCallHangup(IMXCall.END_CALL_REASON_PEER_HANG_UP);
                }

            } else { // event from the current member, but sent from another device
                switch (eventType) {
                    case EventType.CALL_INVITE:
                        // warn in the UI thread
                        mUIThreadHandler.post(() -> dispatchOnStateDidChange(CALL_STATE_RINGING));
                        break;

                    case EventType.CALL_ANSWER:
                        // call answered from another device
                        mUIThreadHandler.post(this::onAnsweredElsewhere);
                        break;

                    case EventType.CALL_HANGUP:
